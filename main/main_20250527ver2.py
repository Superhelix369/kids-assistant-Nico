...
DEV_MODE_FILE = 'xxxxxxxxxxxxxxxxxxxxxxxxxx'
# ===========================
...
def cleanup_gpio():
    ...
    try:
        ...
        print("⚠ lgpio.gpiochip_open() エラー:", e)
    else:
        try:
            lgpio.gpiochip_close(handle)
            print("🔓 lgpio による GPIO 解放OK")
        except Exception as e:
            print("⚠ lgpio.gpiochip_close() エラー:", e)
        
    try:
        my_pid = str(os.getpid())
        pids_output = subprocess.run(["sudo", "lsof", "-t", "xxxxxxxxxxxxxxx"], capture_output=True, text=True)
...
def main():
    ...
    try:
        ...
        if not is_dev_mode():
            host = start_ec2()
            if not host:
                print("❌ EC2接続に失敗しました。シャットダウンします。")
                handle_shutdown()
                return
        else:
            print("🛠 開発モード: EC2起動スキップ")
            host = EC2_HOST

        if not wait_for_voicevox(host):
            print("❌ VoiceVox 起動確認失敗。シャットダウンします。")
            handle_shutdown()
            return

        start_assistant(host)

        print("🗣 assistant.py の処理完了を待機中...")
        while state["mode"] != Mode.SHUTTING_DOWN:
            time.sleep(config.SPEAKER_ID)

    except KeyboardInterrupt:
        print("⌨️ Ctrl+C 検出 → シャットダウン処理へ")
        handle_shutdown()
    except Exception as e:
        print(f"⚠️ 予期せぬエラー: {e}")
        traceback.print_exc()
