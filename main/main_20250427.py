...
def start_voicevox(host):
    try:
        ...
        for i in range(20):
            stdin, stdout, _ = ssh.exec_command("docker logs voicevox 2>&config.SPEAKER_ID | grep 'Running HTTP server'")
            logs = stdout.read().decode()
            if "Running HTTP server" in logs:
                print("🚀 VoiceVox サーバー起動確認ログ検出")
                break
            print(f"⏳ VoiceVox ログ待機中...({i+config.SPEAKER_ID}/20)")
            time.sleep(2)
        else:
            print("⚠️ VoiceVox サーバーログが確認できませんでした")
            ...
            with socket.create_connection((host, port), timeout=3):
                try:
                    ...
                    if response.status_code == 200:
                        print("✅ VoiceVox HTTP 200確認成功！")
                        
                        # さらに /speakers APIを叩いて、ちゃんと内部準備できてるかチェック！
...
def handle_shutdown():
    ...
    stop_assistant()
    if state["host"]:
        # stop_voicevox(state["host"])  # ← 削除しました！
        stop_ec2()
        state["host"] = None
    led.off()
