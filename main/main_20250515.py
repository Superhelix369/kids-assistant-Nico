...
DEV_MODE_FILE = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
# ===========================
...
def cleanup_gpio():
    print("🪝 GPIO解放中...")
    try:
        if 'button' in globals() and isinstance(button, Button):
            button.when_pressed = None
            button.when_released = None
            button.when_held = None
            button.hold_time = None
            time.sleep(0.5) 
            try:
                button.close()
        ...
        time.sleep(1.0)
        return #gpiozero 解放に成功した場合はここで終了
    except Exception as e:
        print("⚠ gpiozero 解放エラー:", e)
    ...
    try:
        ...
        pids_output = subprocess.run(["sudo", "lsof", "-t", "xxxxxxxxxxxx"], capture_output=True, text=True)
        pids = [pid for pid in pids_output.stdout.strip().splitlines() if pid != my_pid]
        for pid in pids:
            print(f"⚠ GPIO 使用中のプロセスを強制終了: PID={pid}")
            subprocess.run(["sudo", "kill", "-9", pid], check=False)
        subprocess.run(["sudo", "systemctl", "stop", "lgpio"], check=False)
        subprocess.run(["sudo", "killall", "-9", "lgpiod"], check=False)
...
def handle_shutdown():
    ...
    try:
        with open(config.SHUTDOWN_LOG_PATH, "a") as f:
            f.write(f"[{datetime.now()}] cleanup_gpio() 実行\n")
            f.write(f"[{datetime.now()}] stop_pi() 呼び出し\n\n")
    except Exception as e:
        print(f"⚠️ ログ書き込み失敗: {e}")
