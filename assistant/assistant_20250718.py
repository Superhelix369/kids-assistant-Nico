...
# VoiceVox のホスト（Elastic IP）をコマンドライン引数で受け取る
if len(sys.argv) < 2:
    print("❌ エラー: VoiceVox のホスト（Elastic IP）をコマンドライン引数で指定してください。")
...
def play_audio(audio_data, factor=1.2):
    ...
    with open("temp.wav", "wb") as f:
        f.write(amplified)
    # USBスピーカー（card 3, device 0）で再生
    subprocess.run(["aplay", "-D", "plughw:3,0","-r", "24000", "-f", "S16_LE", "-c", config.SPEAKER_ID, "temp.wav"], check=True)
